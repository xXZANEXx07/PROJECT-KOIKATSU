...
def process_png():
    global path, png_list, kks_folder2, kks_card_list, window, png_count, png_count_t
    ...
    if count > 0:
        ...
        # Solo crear la carpeta de conversión
        if not os.path.isdir(target_folder2):
            os.mkdir(target_folder2)

        # Solo convertir las cartas, sin moverlas
        for card in kks_card_list:
            source = os.path.normpath(os.path.join(path, card))
            convert_kk(card, source, target_folder2)

        messagebox.showinfo("Done", f"[{count}] cards converted\nSaved to [{kks_folder2}] folder\nOriginal KKS cards remain in place")
    else:
...
window = Tk()
window.title("KKSCF - Convert Only")
w = 300
h = 300
...
tkinter.Label(window, text="Converts KKS cards to KK\nKeeps originals in place", pady=10, fg="blue").pack()
